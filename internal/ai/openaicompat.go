...
// call makes an HTTP POST to the chat/completions endpoint.
func (p *openAICompatProvider) call(ctx context.Context, system, user string) (string, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		return "", fmt.Errorf("ai: %s: %s", resp.Status, excerpt)
	}

	var chatResp chatResponse
	if err := json.Unmarshal(respBody, &chatResp); err != nil {
		return "", fmt.Errorf("ai: parse response: malformed JSON: %w", err)
	}
